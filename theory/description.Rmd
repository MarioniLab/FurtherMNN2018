...
In such cases, the batch vector will be of near-zero length and point in some random direction such that orthogonalization may end up removing geniune biology.
...
In practice, this is unlikely to be a major problem as a random vector is still more likely than not to be orthogonal to the biological subspace;
and if this is not the case, we should be able to observe a large loss of variance that indicates that `fastMNN()` should not be run.
