...
In the original expression space, we have assumed that $W$ is orthogonal to $\mathbf{B}$, i.e., $W\mathbf{B} = 0$.
...
The key assumption is that $\mathbf{V}_k$ "captures" the entirety of $W$, 
i.e., there is some linear combination $A$ of the column vectors of $\mathbf{V}_k$ such that $A\mathbf{V}_{k}^T=W$.
Note that this also means that $A = W\mathbf{V}_k$, due to the orthonormality of the column vectors of $\mathbf{V}_k$.
...
The rotated batch vector becomes $W_k = W \mathbf{V}_k$.
We see that 

$$
W_k \mathbf{B}_k = W\mathbf{V}_k \mathbf{V}_k^T \mathbf{B} = A \mathbf{V}_k^T \mathbf{B} = W \mathbf{B} = 0 \;,
$$
